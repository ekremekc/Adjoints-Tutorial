...
diff = Constant(mesh, PETSc.ScalarType(abs(u_max-f_max)))
v, q = TrialFunction(V), TestFunction(V)
a_a = - kappa * dot(grad(v), grad(q)) * dx
L_a = dot(diff, p) * dx

# Solve direct problem
problem_adjoint = LinearProblem(a_a, L_a, bcs=bcs, petsc_options={"ksp_type": "preonly", "pc_type": "lu"})
